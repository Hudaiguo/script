...
class CRNN(object):
    ...
    def _conv2d(self,inputs,filters,padding,batch_norm,name):
        ...
        kernel_initializer = tf.contrib.layers.variance_scaling_initializer()  #或者简写为tf.TruncatedNormal()生成截断正态分布的随机数，这个初始化方法好像在tf中用得比较多。
        bias_initializer = tf.constant_initializer(value=0.0)   #常量初始化
